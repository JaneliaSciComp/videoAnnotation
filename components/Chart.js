...
import {
  predefinedColors,
  staticVerticalLineColor,
  dynamicVerticalLineColor,
  staticVerticalLine,
  dynamicVerticalLine,
} from "../utils/utils";
...
export default function MyChart(props) {
  ...
  useEffect(() => {
    ...
    if (props.metrics?.length > 0 && Object.keys(props.data).length > 0) {
      ...
      endNeeded =
        frameNum + range < totalFrameCount
          ? frameNum + range
          : totalFrameCount - 1;
      for (let i = startNeeded + 1; i <= endNeeded + 1; i++) {
        frameNums.push(i.toString());
      }
      const [startBuffered, ] = props.data[props.metrics[0]].range;
        ...
        datasets: props.metrics.map((m, i) => {
          ...
          return {
            ...
            borderColor: dataset?.borderColor
              ? dataset?.borderColor
              : colorsCopy[i % colorsCopy.length],
            backgroundColor: dataset?.backgroundColor
              ? dataset?.backgroundColor
              : colorsCopy[i % colorsCopy.length] + "80",
    ...
    setOptions({
      ...
      plugins: {
        ...
        staticVerticalLine: {
          ...
          color: props.staticVerticalLineColor
            ? props.staticVerticalLine
            : staticVerticalLineColor,
        },
        dynamicVerticalLine: {
          metricsNumber: props.metrics.length,
          color: props.dynamicVerticalLineColor
            ? props.dynamicVerticalLineColor
            : dynamicVerticalLineColor,
  ...
  function generateChart() {
    switch (props.type) {
      case "Line":
        return <Line ref={chartRef} options={options} data={dataToDisplay} />;
      case "Bar":
        return <Bar ref={chartRef} options={options} data={dataToDisplay} />;
    }
  }
