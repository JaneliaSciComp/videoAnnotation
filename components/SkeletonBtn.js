...
export default function SkeletonBtn(props) {
  ...
  useEffect(() => {
    if (
      !drawType ||
      drawType !== "skeleton" ||
      annotationIdRef.current !== annoIdToDraw
    ) {
      setClicked(false);
  ...
  function addRadioToAnnotation(value) {
    if (clicked) {
      frameAnnotation[annotationIdRef.current].data[skeletonLandmark][2] =
        value;
    }
  }
  ...
  function clickHandler() {
    if (Number.isInteger(frameNum) || frameUrl) {
      ...
      setRadioValue(2);
      const initData = props.data.map(() => [null, null, 2]);
      const annoObj = {
        id: id,
        videoId: videoId,
        type: "skeleton",
    ...
    <div className="d-flex align-items-center">
      <Button
        ...
        style={{
          color: clicked
            ? "white"
            : props.data[skeletonLandmark ? skeletonLandmark : 0].color,
          background: clicked
            ? props.data[skeletonLandmark ? skeletonLandmark : 0].color
            : "white",
          border:
      ...
      >
        {}
        {Number.isInteger(skeletonLandmark)
          ? `${skeletonLandmark + 1} ${props.data[skeletonLandmark].label}`
          : props.skeletonName}
      </Button>
