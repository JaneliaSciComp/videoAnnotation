...
export default function CategoryEraser(props) {
  ...
  useEffect(() => {
    if (cancelIntervalErasing) {
      endIntervalErasing().then(() => {
        ...
        erasingCopy[props.btnGroupId] = {
          ...intervalErasing[props.btnGroupId],
          on: false,
          videoId: null,
          startFrame: null,
        };
  ...
  function clickHandler() {
    if (Number.isInteger(frameNum) || frameUrl) {
      ...
      setIntervalErasing(() => erasingCopy);
  ...
  function endIntervalErasing() {
    ...
    if (erasingObj && Number.isInteger(erasingObj?.startFrame)) {
      const lastFrameNum = frameNum
        ? frameNum
        : lastFrameNumForIntervalErasingRef.current;
  ...
  function eraseFromCurrentFrame() {
    if (
      ...
      Object.keys(frameAnnoCopy).forEach((id) => {
        if (
          frameAnnoCopy[id].type === "category" &&
          erasingObj.labels.includes(frameAnnoCopy[id].label)
        ) {
  ...
  return (
    <>
      <Button
        className={[styles.categoryEraserBtn, styles.btn]}
        size="sm"
        variant="light"
        style={{
          color: intervalErasing[props.btnGroupId]?.on
            ? "white"
            : "rgb(100, 100, 100)",
          background: intervalErasing[props.btnGroupId]?.on
            ? (props.color ?? defaultColor)
            : "white",
          border: intervalErasing[props.btnGroupId]?.on
            ? "1px solid" + (props.color ?? defaultColor)
            : "1px solid rgb(100, 100, 100)",
        }}
        onClick={clickHandler}
        disabled={Object.entries(intervalErasing).some(
          ([id, obj]) => props.btnGroupId !== id && obj.on,
        )}
      >
        <ClearOutlined />
      </Button>
