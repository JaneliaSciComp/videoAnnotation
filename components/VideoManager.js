...
export default function VideoManager(props) {
    ...
    if (props.additionalFields?.length > 0) {
      ...
      for (let field of props.additionalFields) {
        ...
        if (field.loadIn === "canvas") {
          if (!field.onLoad) {
            throw new Error(
              "If loadin is 'canvas', then attribute onLoad is required",
            );
          }
  ...
  function onDelBtnClick(i) {
    ...
    Modal.confirm({
      title: "Alert",
      content:
        "The current video data including configuration and annotations will be removed!",
      onOk: async () => {
        await deleteThisVideo(videoIdToDel);
      },
    });
  }
        ...
        <div>
          ...
          >
            ...
            <Form className="my-4 mx-3" form={form} size="small">
              ...
              >
                <Input onChange={onVideoPathChange} allowClear />
              </Form.Item>
              ...
              >
                <Input onChange={onVideoNameChange} allowClear />
              </Form.Item>

              {props?.additionalFields?.length > 0 ? (
                <p className="mt-2 mb-4" style={{ fontWeight: "bold" }}>
                  Additional Data
                </p>
              ) : null}

              {props?.additionalFields?.length > 0
                ? props.additionalFields.map((params, i) => (
                    <Form.Item
                      key={i}
                      name={params.name}
                      label={params.label}
                    ...
                    >
                      <Input
                        onChange={(e) => {
                          onAdditionalFieldChange(e, params.name);
                        }}
                        allowClear
                      />
                    </Form.Item>
              ...
              <div className="d-flex justify-content-center">
                <Space size="small">
                  <Button onClick={onAddBtnClick} disabled={btnDisable}>
                    Add
                  </Button>
                  <Button onClick={onAddLoadBtnClick} disabled={btnDisable}>
                    Add and Load
                  </Button>
                  <Button onClick={onEditBtnClick} disabled={btnDisable}>
                    Edit
                  </Button>
                </Space>
              </div>
